...
import os, src.calculations as calculations
...
def get_sub_total():
    ...
    # Calls the module 'calculations' to use 'principal_Calculation' and calculate
      the main values using 'sub_total_float' as an argument and storage them into 2 separate variables.
     
    # Calls the module 'calculations' to use 'GUI_principal_Calculation' and storage the returned 
      text value on the 'principal_calculation' variable, using the previous variables 'sub_total_float', 'IVA', 'total'.
    
    # Creates a Label to print the 'GUI_principal_Calculation' returned text, storage into 'principal_calculation' variable.
      The label contains its own configuration such as font, border, width, height, relief. And its put it into the window 
      using grid.
    
    # 2 tkinter.messageboxes, asking a yes/no question are created saving their values into the 2 global variables for further use.
    
    # Now depending on the values of the tkinter.messageboxes the program would ask using a simpledialog for a value, which is goin
      to be saved as a float.
      Then we again call the calculations module this time to use the 'calc_IGTF' function, and save its value to the variable 'IGTF'.
      We call again the calculations module, this time to calculate the retention of the values. 
      And save the return values into 'reten_IVA' & 'reten_Total'.
      With those 2 values we then call again the calculations module, to use the 'GUI_print_Reten_Bill', using as arguments 
      the previous variables (sub_total_float, reten_IVA, pay_cash, IGTF, reten_Total), saving the return value into 'reten_result'.
    ...
    # The rest of the function is the same but just depending on the values of the 'main_question' & 'agent_of_retention'.
      It would use the module calculations 'GUI_print_Reten_Bill' or 'GUI_print__Bill'.
      Saving an individual 'my_list' for each case, to be use further into the program.
...
def clicked(): 
    ...
    Case 1 'agent_of_retention' == 'no':
        ...
        to use the 'convert_to_bolivars' function using the variables 'my_list' & 'calculation.price_BCV' as arguments,
    ...
    if agent_of_retention == 'no':
        sub_total_BS,  IVA_BS, pay_cash_BS, IGTF_BS, total_BS = calculations.convert_to_Bolivars(my_list , calculations.price_BCV)
...
version = Label(window, text='Version: alpha-1.0')
